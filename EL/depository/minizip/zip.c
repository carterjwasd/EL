...
typedef struct {
  ...
  uLong
      size_centralextrafree; /* Extra bytes allocated to the central header but that are not used */
  uLong size_comment;
...
#define zip64local_in_range(min, max, value) ((min) <= (value) && (value) <= (max))
  /* Years supported:
    ...
    */
  if (!(zip64local_in_range(1980, 2107, ptm->tm_year) ||
        zip64local_in_range(0, 207, ptm->tm_year)) ||
      !zip64local_in_range(0, 11, ptm->tm_mon) || !zip64local_in_range(1, 31, ptm->tm_mday) ||
      !zip64local_in_range(0, 23, ptm->tm_hour) || !zip64local_in_range(0, 59, ptm->tm_min) ||
...
local int zip64local_putValue OF(
...
local int zipGoToSpecificDisk(zipFile file, int number_disk, int open_existing) {
  ...
  zi->filestream = ZOPENDISK64(
      zi->z_filefunc, zi->filestream_with_CD, number_disk,
      (open_existing == 1)
...
local ZPOS64_T zip64local_SearchCentralDir(const zlib_filefunc64_32_def* pzlib_filefunc_def,
  ...
  while (back_read < max_back) {
    ...
    read_size = ((BUFREADCOMMENT + 4) < (file_size - read_pos)) ? (BUFREADCOMMENT + 4)
                                                                : (uLong)(file_size - read_pos);
...
extern zipFile ZEXPORT zipOpen4(const void* pathname,
  ...
  if (append == APPEND_STATUS_ADDINZIP) {
    ...
    if (err == ZIP_OK) {
      ...
      if (zip64local_getShort(&ziinit.z_filefunc, ziinit.filestream, &ziinit.number_disk_with_CD) !=
          ZIP_OK)
        err = ZIP_ERRNO;
      ...
      if ((err == ZIP_OK) && ((number_entry_CD == 0xffff) || (offset_central_dir == 0xffffffff))) {
        ...
        central_pos =
            zip64local_SearchCentralDir64(&ziinit.z_filefunc, ziinit.filestream, central_pos);

        if (central_pos) {
          ZPOS64_T sizeEndOfCentralDirectory;

          if (ZSEEK64(ziinit.z_filefunc, ziinit.filestream, central_pos, ZLIB_FILEFUNC_SEEK_SET) !=
              0)
            err = ZIP_ERRNO;
          ...
          if (zip64local_getLong64(&ziinit.z_filefunc, ziinit.filestream,
          ...
          if (zip64local_getShort(&ziinit.z_filefunc, ziinit.filestream, &uL) != ZIP_OK)
            err = ZIP_ERRNO;
          /* number of this disk */
          if (zip64local_getLong(&ziinit.z_filefunc, ziinit.filestream, &ziinit.number_disk) !=
              ZIP_OK)
            err = ZIP_ERRNO;
          /* number of the disk with the start of the central directory */
          if (zip64local_getLong(&ziinit.z_filefunc, ziinit.filestream,
                                 &ziinit.number_disk_with_CD) != ZIP_OK)
            err = ZIP_ERRNO;
          /* total number of entries in the central directory on this disk */
          if (zip64local_getLong64(&ziinit.z_filefunc, ziinit.filestream, &number_entry) != ZIP_OK)
            err = ZIP_ERRNO;
          /* total number of entries in the central directory */
          if (zip64local_getLong64(&ziinit.z_filefunc, ziinit.filestream, &number_entry_CD) !=
              ZIP_OK)
            err = ZIP_ERRNO;
          if (number_entry_CD != number_entry)
            err = ZIP_BADZIPFILE;
          /* size of the central directory */
          if (zip64local_getLong64(&ziinit.z_filefunc, ziinit.filestream, &size_central_dir) !=
              ZIP_OK)
            err = ZIP_ERRNO;
          /* offset of start of central directory with respect to the starting disk number */
          if (zip64local_getLong64(&ziinit.z_filefunc, ziinit.filestream, &offset_central_dir) !=
              ZIP_OK)
            err = ZIP_ERRNO;
        }
        else
    ...
    if (size_comment > 0) {
      ...
      if (ziinit.globalcomment) {
        size_comment =
            ZREAD64(ziinit.z_filefunc, ziinit.filestream, ziinit.globalcomment, size_comment);
    ...
    if (ZSEEK64(ziinit.z_filefunc, ziinit.filestream, offset_central_dir + byte_before_the_zipfile,
                ZLIB_FILEFUNC_SEEK_SET) != 0)
      err = ZIP_ERRNO;
...
extern int ZEXPORT zipOpenNewFileInZip4_64(zipFile file,
  ...
  zi->ci.central_header =
      (char*)ALLOC((uInt)zi->ci.size_centralheader + zi->ci.size_centralextrafree + size_comment);
  ...
  zip64local_putValue_inmemory(zi->ci.central_header + 34, (uLong)zi->ci.number_disk,
                               2); /*disk nm start*/
  ...
  if ((err == ZIP_OK) && (size_extrafield_local > 0)) {
    if (ZWRITE64(zi->z_filefunc, zi->filestream, extrafield_local, size_extrafield_local) !=
  ...
  if ((err == Z_OK) && ((zi->ci.flag & 1) != 0)) {
    ...
    if (zi->ci.method == AES_METHOD) {
      ...
      fcrypt_init(AES_ENCRYPTIONMODE, password, strlen(password), saltvalue, passverify,
                  &zi->ci.aes_ctx);

      if (ZWRITE64(zi->z_filefunc, zi->filestream, saltvalue, saltlength) != saltlength)
        err = ZIP_ERRNO;
      if (ZWRITE64(zi->z_filefunc, zi->filestream, passverify, AES_PWVERIFYSIZE) !=
          AES_PWVERIFYSIZE)
        err = ZIP_ERRNO;
...
extern int ZEXPORT zipOpenNewFileInZip4(zipFile file,
  ...
  return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
                                 ...
                                 raw, windowBits, memLevel, strategy, password, crcForCrypting,
                                 versionMadeBy, flagBase, 0);
}
...
extern int ZEXPORT zipOpenNewFileInZip2(zipFile file,
  ...
  return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
                                 extrafield_global, size_extrafield_global, comment, method, level,
                                 raw, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, NULL, 0,
                                 VERSIONMADEBY, 0, 0);
}
...
local int zip64FlushWriteBuffer(zip64_internal* zi) {
  ...
  do {
    ...
    written =
        ZWRITE64(zi->z_filefunc, zi->filestream, zi->ci.buffered_data + total_written, max_write);
...
extern int ZEXPORT zipClose(zipFile file, const char* global_comment) {
  ...
  if (pos >= 0xffffffff || zi->number_entry > 0xffff) {
    ...
    /* number of the disk with the start of the central directory */
    if (err == ZIP_OK)
      err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_disk_with_CD + 1,
                                4);
  }
  ...
  if (err == ZIP_OK && size_global_comment > 0) {
    if (ZWRITE64(zi->z_filefunc, zi->filestream, global_comment, size_global_comment) !=
        size_global_comment)
      err = ZIP_ERRNO;
  }
