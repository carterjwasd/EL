...
voidpf ZCALLBACK win32_open64_file_func(voidpf opaque, const void* filename, int mode) {
  ...
  win32_translate_open_mode(mode, &dwDesiredAccess, &dwCreationDisposition, &dwShareMode,
...
#ifdef UNICODE
    hFile =
        CreateFile2((LPCTSTR)filename, dwDesiredAccess, dwShareMode, dwCreationDisposition, NULL);
#else
...
#ifdef IOWIN32_USING_WINRT_API
    hFile =
        CreateFile2((LPCWSTR)filename, dwDesiredAccess, dwShareMode, dwCreationDisposition, NULL);
#else
...
voidpf ZCALLBACK win32_opendisk64_file_func(voidpf opaque,
                                            voidpf stream,
                                            int number_disk,
                                            int mode) {
  WIN32FILE_IOWIN* iowin = NULL;
...
voidpf ZCALLBACK win32_opendisk64_file_funcW(voidpf opaque,
                                             voidpf stream,
                                             int number_disk,
                                             int mode) {
  WIN32FILE_IOWIN* iowin = NULL;
...
voidpf ZCALLBACK win32_opendisk64_file_funcA(voidpf opaque,
                                             voidpf stream,
                                             int number_disk,
                                             int mode) {
  WIN32FILE_IOWIN* iowin = NULL;
...
static BOOL win32_setfilepointer_internal(HANDLE hFile,
                                          LARGE_INTEGER pos,
